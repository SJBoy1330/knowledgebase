...
class Action_m extends CI_Model
{
    ...
    public function get_single($tabel, $array = NULL)
    {
    ...
    public function get_all($tabel, $array = NULL, $order = NULL)
    {

        if ($array != NULL) {

            $this->db->from($tabel)->where($array);
        } else {
    ...
    public function delete_batch($tabel, $where = null)
    {
        ...
        if ($where != null) {
            if (isset($where)) {
                $this->db->where_in($where['kolom'], $where['value']);
            }
            return $this->db->delete($tabel);
        } else {
            return NULL;
